...
(define (add-assertion! assertion)
  ...
  (let ((old-assertions THE-ASSERTIONS))
    ...
    'ok))
(define (add-assertion-exp! assertion-exp)
  (add-assertion! (query-syntax-process assertion-exp)))
(define (add-assertion-exps! assertion-exps)
  (if (null? assertion-exps)
      'ok
      (begin (add-assertion-exp! (car assertion-exps))
             (add-assertion-exps! (cdr assertion-exps)))))
...
(define (add-rule! rule)
  ...
  (let ((old-rules THE-RULES))
    ...
    'ok))
(define (add-rule-exp! rule-exp)
  (add-rule! (query-syntax-process rule-exp)))
...
(define (qeval-exp qexp)
  (display "QUERY: ")
  (display-line qexp)
  (display-line "[")
  (let ((q (query-syntax-process qexp)))
    (display-stream
     (stream-map
      (lambda (frame)
        (instantiate q
