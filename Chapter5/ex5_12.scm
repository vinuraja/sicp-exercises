  ...
  (let ((pc (make-register 'pc))
    ...
    (let ((the-ops
      ...
      (define (add-inst inst)
        (if (not (member inst deduped-insts))
            (set! deduped-insts (cons inst deduped-insts))))
      (define (add-entry-point-register inst)
        (if (eq? (car inst) 'goto)
                  ...
                  (let ((reg-name (register-exp-reg dest)))
                    (if (not (member reg-name entry-point-registers))
                        (set! entry-point-registers
                              (cons reg-name entry-point-registers))))))))
      (define (add-stacked-register inst)
        (if (or (eq? (car inst) 'save)
                (eq? (car inst) 'restore))
            (let ((reg-name (stack-inst-reg-name inst)))
              (if (not (member reg-name stacked-registers))
                  (set! stacked-registers
                        (cons reg-name stacked-registers))))))
      ...
      (define (analyze-instruction-sequence)
        (for-each (lambda (inst)
                    (add-inst (car inst))
                    (add-entry-point-register (car inst))
                    (add-stacked-register (car inst))
                    (add-assignment-source (car inst)))
                  the-instruction-sequence)
        (set! deduped-insts
              (rkt:sort (rkt:stream->list (rkt:sequence->stream deduped-insts))
                        (lambda (l r)
                          (rkt:symbol<? (car l) (car r))))))
      (define (dispatch message)
...
(define (get-sorted-insts machine)
  (machine 'sorted-insts))
(define (get-entry-point-registers machine)
  (machine 'entry-point-registers))
(define (get-stacked-registers machine)
  (machine 'stacked-registers))
(define (get-assignment-sources machine reg-name)
  ((machine 'assignment-sources) reg-name))
...
(set-register-contents! correct-fib-machine 'n 3)
(start correct-fib-machine)
(check-equal? 2 (get-register-contents correct-fib-machine 'val))

(get-sorted-insts correct-fib-machine)
(check-equal? '(continue) (get-entry-point-registers correct-fib-machine))
(check-equal? '(val n continue) (get-stacked-registers correct-fib-machine))
(for-each (lambda (reg-name)
            (display reg-name)
            (display ": ")
            (display (get-assignment-sources correct-fib-machine reg-name))
            (newline))
          (get-stacked-registers correct-fib-machine))